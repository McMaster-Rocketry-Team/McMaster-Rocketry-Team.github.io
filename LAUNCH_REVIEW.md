...
**Fixed 2026-08-29:** the fixed nav (`nav{position:fixed}`, `site.css:103`) used a top-down gradient scrim that faded to fully transparent by the bottom of the bar (`site.css:107-114`). On pages where a light "paper" section scrolls up directly behind the nav (`/subteams`, `/members`), the bottom of the nav, exactly where the tagline and nav links sit, had zero scrim and page content showed through, worst case a card heading doubling with the "GET BLASTED" tagline on `/subteams`. Fixed by raising the `data-scrolled` gradient's floor from `0` to `.55`, so it still fades (not a flat solid bar) but never hits full transparency.
...
- **Fixed 2026-08-29:** `logo-mark-white-512.png`, previously the apple-touch-icon, was a white mark on a fully transparent background, near-invisible on a light iOS home screen. `apple-touch-icon` now points at a new `public/brand/apple-touch-icon.png`: the dark-mark `logo-mark-black-red-512.png` (graphite-900 arc, red M) flattened onto an opaque `--paper` (`#F7F7F7`) background at 180x180. The webmanifest's own icon entry is unchanged (`logo-mark-white-512.png` on transparent), that one only ever renders against the manifest's own `background_color:#1A1A1A`, which is dark, so it isn't the same risk.
...
- [x] Consent from every person in `src/data/members.json` (full names, programme, year, headshots for the whole leads roster): confirmed by Robin 2026-08-29, everyone consented.
